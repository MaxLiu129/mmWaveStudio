...
ang_exp=5
ang_base=10**ang_exp
NUM_TX = 3              ### based on config ###
NUM_RX = 4              ### based on config ###
CHIRP_LOOPS=128
ADC_SAMPLES=256
gpu_device=torch.device('cpu')
NUM_TRX=3*4             ### based on config ###
x_bin_size=128
y_bin_size=128
...
def preprocess_batch_length_data(data, steer_mat, r_idx_mat):
    ...
    with torch.no_grad(): # .no_grad() to reduce memory consumption for computations
        data = data.cfloat().to(gpu_device)
        frame_size=data.shape[0]  # frame_size == batch_length?
        frames_tensor=data
...
if __name__=='__main__':
    frame_size = 100
    trial_list=['dumpster']  ### output directory ###
    root_loc = './testingnpy_1/'  ### input directory ###
    begin=0
    end=20
    ...
    for trial in trial_list:
        ...
        data_tensor=torch.from_numpy(np.asarray(data)[None])#1*15*1*2*128*256
        channel_doppler, channel_angle=preprocess_batch_length_data(data_tensor, steer_mat, r_idx_mat)
        
        ### plot fusion to prepare LSTM input tensor ###
        
        ## 
        
        print('channel angle: {}'.format(channel_angle.shape))
        print('channel doppler (rv): {}'.format(channel_doppler.shape))
        print('channel doppler example: {}'.format(channel_doppler[0]))
        
        for i, (hm, hm2) in enumerate(zip(channel_angle, channel_doppler)):
            hm=hm.cpu().numpy()
            hm2=hm2.cpu().numpy()   
            plt.figure(0)   # ??? what're the axes???
            plt.matshow(hm,vmin=-1,vmax=1)
            plt.savefig(outloc+'angle{}.jpg'.format(i))
            plt.figure(1)
            plt.matshow(hm2,vmin=-1,vmax=1)
